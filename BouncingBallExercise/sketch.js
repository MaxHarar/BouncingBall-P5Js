 ...
 *
 * 3. 2022-12-14: Changed the text box so it is in the upper center of the screen,
 *      added red coloring, and changed Hello World to Happy Holidays and 
 *      centered the text accordingly. 
   
   4. Changed Ball generation so they do not get stuck on the edges of the screen and changed it so when you click, all balls get a new color 
   
   5. Revises collision checking for the sides of the box to allow them to get into the 
       top left and right corners while still bouncing off the edges. 
 *
...
function preload() {
  ...
  // bg = loadImage('desktop/christmasBackground.jpeg');
}
...
function createBox() {
  ...
  textAlign(CENTER);
  text('Happy Holidays!', width/2, BOX_HEIGHT/2);   
...
class Ball { // Constructor
  ...
  constructor() {
    // initial position
    this.ballX = random(200, width-50)
    this.ballY = random(200, height-50)
...
//    if (((this.ballX-radius) < BOX_WIDTH && (this.ballY-radius) < BOX_HEIGHT) || d < radius) {
    if (((this.ballX+radius) > BOX_WIDTH/2 && (this.ballX-radius < BOX_WIDTH/2 + width/2) && (this.ballY-radius) < BOX_HEIGHT)) {
